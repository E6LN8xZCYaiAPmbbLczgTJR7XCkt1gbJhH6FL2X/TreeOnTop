...
local Remotes = {
	MuscleEvent = PlayerData.Player:WaitForChild("muscleEvent")
}
...
Tabs.Main:AddButton("Load Anti Lag", function()
...
local ShowPetsOtherPets = Tabs.Main:AddSwitch("Show Pets & Other Pets", function(bool)
    local hidePets = PlayerData.Player:FindFirstChild("hidePets")
    local showOtherPets = PlayerData.Player:FindFirstChild("showOtherPetsOn")
    if hidePets then
        hidePets.Value = bool
    end
    if showOtherPets then
        showOtherPets.Value = bool
    end
end)
ShowPetsOtherPets:Set(false)

local frameSwitch = Tabs.Main:AddSwitch("Hide Frames", function(bool)
...
local statPetDropdown = Tabs.Killing:AddDropdown("Select Pack Pet", function(text)
...
local DisableEggSwitch = Tabs.Killing:AddSwitch("Block Eating Eggs", function(bool)
...
Tabs.Killing:AddSwitch("Egg + Nan Combo", function(bool)
...
local function equipPunch()
	...
	local punch = backpack:FindFirstChild("Punch")
		or char:FindFirstChild("Punch")
	if punch and punch:IsA("Tool") and punch.Parent ~= char then
...
if PlayerData.Character then
	onCharacterAdded(PlayerData.Character)
end

local PunchData = {AutoPunching = false}

Tabs.Killing:AddSwitch("Auto Punch [Without Animation]", function(enabled)
	...
	if enabled then
		task.spawn(function()
			...
			if not tool then repeat tool = PlayerData.Backpack:FindFirstChild("Punch") or PlayerData.Character:FindFirstChild("Punch") task.wait(0.01) until tool end
...
local blacklistDropdown = Tabs.Killing:AddDropdown("Add to Blacklist", function(selectedText)
	local playerName = selectedText:match("| (.+)$")
	if playerName then
		playerName = playerName:gsub("^%s*(.-)%s*$", "%1")
...
local specsdropdown = Tabs.Killing:AddDropdown("Select Player to Spectate", function(text)
...
Tabs.Specs:AddLabel("Stats:").TextSize = 28
...
local playerNameLabel = Tabs.Specs:AddLabel("Selected User: N/A")
playerNameLabel.TextSize = 28
playerNameLabel.TextColor3 = Red
...
local AdvancedStats = {
	HealthLabel = Tabs.Specs:AddLabel("Enemy Health: N/A"),
	EnemyDamageLabel = Tabs.Specs:AddLabel("Enemy Damage: N/A"),
	PlayerHealthLabel = Tabs.Specs:AddLabel("Your Health: N/A"),
	PlayerDamageLabel = Tabs.Specs:AddLabel("Your Damage: N/A"),
	HitsToKillLabel = Tabs.Specs:AddLabel("Hits to Kill: N/A")
}

AdvancedStats.HealthLabel.TextColor3 = Green
AdvancedStats.EnemyDamageLabel.TextColor3 = Green
AdvancedStats.PlayerHealthLabel.TextColor3 = Red
AdvancedStats.PlayerDamageLabel.TextColor3 = Red
AdvancedStats.HitsToKillLabel.TextColor3 = Red
...
local rEvents = Services.ReplicatedStorage:WaitForChild("rEvents")
local equipPetEvent = rEvents:WaitForChild("equipPetEvent")

local petsFolder = PlayerData.Player:WaitForChild("petsFolder")
...
if PlayerData.Character then
	currentHum = PlayerData.Character:FindFirstChildOfClass("Humanoid")
end

Tabs.Godmode:AddSwitch("Pack Spam", function(b)
	isSpamming = b
...
for _, size in ipairs({1, 2}) do
    Tabs.Godmode:AddButton("Size " .. size, function()
...
local function startKillerFollow()
    ...
    killerFollowConnection = Services.RunService.Heartbeat:Connect(function()
        local target = killerTargetPlayer
        local localPlayer = getLocalPlayer()

        if not killerFollowEnabled or not target then
...
local function refreshPlayerDropdown()
    ...
    for _, player in ipairs(Services.Players:GetPlayers()) do
        if player ~= getLocalPlayer() then
            ...
            playerMap[text] = player
            playerDropdown:Add(text)
        end
    end
end

local function createPlayerDropdown()
    playerDropdown = Tabs.Godmode:AddDropdown("Select Player", function(selectedText)
...
Services.Players.PlayerAdded:Connect(refreshPlayerDropdown)
Services.Players.PlayerRemoving:Connect(refreshPlayerDropdown)

Tabs.Godmode:AddSwitch("Teleport to Player", function(state)
...
Tabs.Godmode:AddSwitch("Godmode Kill", function(state)
    godFeaturesActive = state

    if state then
        task.spawn(function()
            while godFeaturesActive do
                local player = PlayerData.Player
                local character = player.Character or PlayerData.Character
                local backpack = player:FindFirstChild("Backpack") or PlayerData.Backpack

                Services.ReplicatedStorage.rEvents.brawlEvent:FireServer("joinBrawl")

                local groundSlam =
                    backpack:FindFirstChild("Ground Slam")
                    or (character and character:FindFirstChild("Ground Slam"))

                if groundSlam then
                    if groundSlam.Parent == backpack and character then
                        groundSlam.Parent = character
                    end

                    local attackTime = groundSlam:FindFirstChild("attackTime")
                    if attackTime then
                        attackTime.Value = 0
                    end

                    Remotes.MuscleEvent:FireServer("slam")
                    groundSlam:Activate()
                end
...
local BoostEaterLabel = Tabs.Inventory:AddLabel("Boost Eater:")
BoostEaterLabel.TextSize = 28
BoostEaterLabel.TextColor3 = Color3.fromRGB(255, 0, 0)

local EggGifterData = {ProteinEggLabel = Tabs.Inventory:AddLabel("Protein Eggs: 0"), SelectedPlayer = nil, EggCount = 0}
EggGifterData.ProteinEggLabel.TextSize = 21
...
ShakeGifterData.TropicalShakeLabel.TextSize = 21
...
Tabs.Inventory:AddSwitch("Eat All Boosts", function(state) BoostData.Running = state end)

Tabs.Inventory:AddLabel("———————————————————————————————————————————")

local EggGifterLabel = Tabs.Inventory:AddLabel("Egg Gifter:")
EggGifterLabel.TextSize = 28
EggGifterLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
...
local ShakeGifterLabel = Tabs.Inventory:AddLabel("Shake Gifter:")
ShakeGifterLabel.TextSize = 28
ShakeGifterLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
