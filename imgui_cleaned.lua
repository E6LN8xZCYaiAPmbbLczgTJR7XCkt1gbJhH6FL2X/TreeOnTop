...
local object = prefabs:FindFirstChild("DropdownButton"):Clone()
					...
					function tab_data:AddConsole(console_options)
						...
						console_options = {
							...
							["source"] = "Lua", -- Only Lua mode supported (Logs mode was incomplete/broken)
						...
						do
							...
							local Highlight = function(str, keywords)
								...
								S = S:gsub(".", function(c)
									if Token[c] then return " " else return c end
								end)
								S = S:gsub("%S+", function(c)
									if K[c] then return c else return (" "):rep(#c) end
								end)
								return S
							end

							local hTokens = function(str)
								local Token = {["="] = true, ["."] = true, [","] = true, ["("] = true, [")"] = true, ["["] = true, ["]"] = true, ["{"] = true, ["}"] = true, [":"] = true, ["*"] = true, ["/"] = true, ["+"] = true, ["-"] = true, ["%"] = true, [";"] = true, ["~"] = true}
								local A = ""
								str:gsub(".", function(c)
							...
							local highlightStrings = function(str)
								local highlight = ""
								local quote = false
								str:gsub(".", function(c)
									...
									else highlight = highlight .. " " end
								end)
								return highlight
							end

							local highlightInfo = function(str)
								local highlight = ""
								local quote = false
								str:gsub(".", function(c)
									if not quote and c == "[" then quote = true
							...
							local highlightComments = function(str)
								local ret = ""
								str:gsub("[^\r\n]+", function(c)
									local comm = false
									local i = 0
									c:gsub(".", function(n)
										...
										if comm then ret = ret .. n else ret = ret .. " " end
									end)
								end)
								return ret
							end

							local highlightNumbers = function(str)
								local A = ""
								str:gsub(".", function(c)
							...
							local highlight_lua = function()
								...
								Source.Numbers.Text = highlightNumbers(s)
								Source.Strings.Text = highlightStrings(s)
								Source.Comments.Text = highlightComments(s)
								Source.Info.Text = highlightInfo(s)
